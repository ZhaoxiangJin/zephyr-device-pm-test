 ...
 * and says nothing about the LPADC. Busy-wait, with interrupts on, for long
 * enough that the last character has left and that interrupt has run: one
 * character takes 1 ms even at 9600 baud, and printk() already waited for every
 * earlier one, so 5 ms is several times the worst case.
 */
...
static void phase_system_managed(void)
{
	 ...
	 * core domain's TURN_ON has already run by the time the LPADC's own
